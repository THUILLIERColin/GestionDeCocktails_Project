<?php 
    ...
    */
    ...
    function intialisationRecettePourCategorie(){
        ...
        file_put_contents('Donnees/initialisation.inc.php', '<?php $recettesParCategorie = '.var_export($tableInit, true).';'); 
    }
...
function searchImageRecette($recette){
    ...
    $nomImage = $recette['titre'];
    $nomImage = strtolower($nomImage); //on met tout en minuscule
    //on remplace tout les caractères avec accent par leur équivalent sans accent
    $nomImage = str_replace(
        array(
            'à', 'â', 'ä', 'á', 'ã', 'å',
            'î', 'ï', 'ì', 'í',
            'ô', 'ö', 'ò', 'ó', 'õ', 'ø',
    ...
    $nomImage = ucfirst($nomImage); //on met la première lettre en majuscule
    $nomImage = str_replace(' ', '_',$nomImage); // on remplace les espaces par des _
    ...
    function importationTempsVersFichier(){
        if(file_exists("Donnees/donneeFav.php")&&filesize("Donnees/donneeFav.php")){  //Si le fichier contenant les recettes aimée existe et n'est pas vide
            include("Donnees/donneeFav.php");                                 //on inclu donneeFav.php pour recuperer l'array $utilisateur qui contient nos données
        ...
        file_put_contents("Donnees/donneeFav.php",'<?php $utilisateur = '.var_export($utilisateur, true).';'.'?>'); // on crée/ajoute au fichier notre array de nom + recette favorite 
    }
    function verifierMdp(){
        //verification si le user existe grace au nom du fichier
        if (file_exists("Donnees/DonneesUtilisateur/".$_POST['loginConnexion'].".txt"))
        {
           
            
            //ouverture du fichier
            $fichier = fopen("Donnees/DonneesUtilisateur/".$_POST['loginConnexion'].".txt", "r");
            //lecture du fichier
            $ligne = fgets($fichier);
            ...
            if (verifyHashMDP($_POST["mdpConnexion"], $tab1[1]))
            {
                ...
                if(isset($_SESSION["favTemp"])&&!empty($_SESSION["favTemp"])){
            ...
            else
            {
                ...
                ?><p class="erreur">Mot de passe incorrect </p><?php
            }         
        }
        else
        {
            //si le nom d'utilisateur n'existe pas on affiche un message d'erreur
            ?><p class="erreur">Nom d'utilisateur incorrect </p><?php
        }
   }

   function retrouverDonneeUserNom(){
       //ouverture du fichier
       $fichier = fopen("Donnees/DonneesUtilisateur/".$_SESSION['user']['login'].".txt", "r");
       //lecture du fichier
       $ligne = fgets($fichier);
       //fermeture du fichier
       fclose($fichier);
